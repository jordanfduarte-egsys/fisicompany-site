...
class Noticia extends MY_Controller{
    ...
    {
        $this->data['noticias'] = $this->consultar();
    ...
    public function deletar()
    {
        if ($this->model->deletar()) {
            $this->session->set_flashdata('msg', 'Notícia removido com sucesso');
        } else {
            $this->session->set_flashdata('erro', 'Erro ao remover a notícia.');
        }
    ...
    public function valida()
    {
        ...
        if($this->form_validation->run() == FALSE) {//Erro na validacao
            ...
            if ((!isset($this->uri->segments[4]) ? $this->model->inserir() : $this->model->editar())) {
                $this->session->set_flashdata('msg', 'Notícia salva com sucesso');
            } else {
                $this->session->set_flashdata('erro', 'Erro ao salvar notícia');
            }
