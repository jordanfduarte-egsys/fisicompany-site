<?php
class Modalidade extends MY_Controller
{

    public function __construct()
    {        //intancia da base
        parent::__construct();    
        $this->data['title'] = "Modalidade";
    ...
    {
        $this->data['modalidades'] = $this->consultar();
    ...
    {
        $this->data['acao'] = "Nova modalidade";
    ...
    {
        $this->data['modalidade'] = $this->conultarInstancia();
        $this->data['acao'] = "Editar modalidade";        
        $this->load->view("admin/modules/".CONTROLLER."/form",$this->data);
        $this->load->view("admin/abstract/footer");
    }
    ...
    public function deletar()
    {
        ...
        if ($this->model->deletar($id)){
            ...
            $this->session->set_flashdata('msg', 'Modalidade removido com sucesso');
        } else {
            $this->session->set_flashdata('erro', 'Erro ao remover a modalidade.');
        }
    ...
    public function valida()
    {
        ...
        if($this->form_validation->run() == FALSE){//Erro na validacao
            ...
            if ((!isset($this->uri->segments[4]) ? $this->model->inserir() : $this->model->editar())) {
                $this->session->set_flashdata('msg', 'Modalidade salva com sucesso');
            } else {
                $this->session->set_flashdata('erro', 'Erro ao salvar a modalidade');
            }
