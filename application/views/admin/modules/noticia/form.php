...
<fieldset class="scheduler-border">
    ...
    <input type="text" tabindex="1" name="titulo"  class="form-control" value="<?php echo (isset($noticia[0]->titulo) ? $noticia[0]->titulo  : set_value("titulo")) ?>"/>
    <?php echo form_error('titulo', '<div class="alert alert-danger">', '</div>'); ?>
    
    <label for="resumo">Resumo</label>
    <input type="text" tabindex="2" name="resumo"  class="form-control" value="<?php echo (isset($noticia[0]->resumo) ? $noticia[0]->resumo  : set_value("resumo")) ?>"/>
    <?php echo form_error('resumo', '<div class="alert alert-danger">', '</div>'); ?>
    
    <label for="texto">Texto</label>
    <textarea name="texto">
        <?php echo (isset($noticia[0]->texto) ? $noticia[0]->texto  : set_value("texto")) ?>
    </textarea>
    ...
    <fieldset class="scheduler-border">
        ...
        <div class="info_uploade">
            <p>
                ...
                <b>    
Importante: </b>Dimensões recomendada 1080 x 300 px.<br/>
                <b>Dica: </b>Arraste e solte a imagem após a upload para ordenar.
            </p>
        </div>        
...
Adicionar imagem</div>
        ...
        <div id="imagens">            
            <?php if($noticia['isEditar']){
                    foreach($noticia as $imagem):
                    ...
                    <div class='imagem_mini'>
                        ...
                        <div class='imagem_mini_favorito'>
                            <a href='javascript:void(0)' class='imagem_mini_favorito_a' title='Adicionar como favorito'>
                                <img class='imagem_mini_favorito_a_img' src='/assets/img/favorito<?php echo $imagem->imagem == $noticia[0]->imagem_principal ?  '': '_over'?>.png'/></a>
                        </div>
                    </div>
                
            <?php   endforeach; }?>            
        </div>
        <input type="hidden" id="imagem_principal" name="imagem_principal" value="<?php echo (isset($noticia[0]->imagem_principal)) ? $noticia[0]->imagem_principal : ""?>"/>
    </fieldset>
    <br/>
    <input type="hidden" name="id_noticia" value="<?php echo (isset($noticia[0]->id_noticia) ? $noticia[0]->id_noticia : "")?>"/>
    <input type="hidden" name="data" value="<?php echo (isset($noticia[0]->data) ? $noticia[0]->data : "")?>"/>
    <input type="hidden" name="contador" value="<?php echo (isset($noticia[0]->contador) ? $noticia[0]->contador : "")?>"/>
    <?php
